...
import { ArrowLeft, Bitcoin, Compass, Users, Fuel, Maximize, BedDouble, Bath, Zap, Gauge, MapPin, Calendar, Building2, ExternalLink } from "lucide-react";
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
        ...
        <div>
          <motion.div 
            ...
            className="rounded-3xl overflow-hidden aspect-square relative shadow-2xl shadow-black/50"
          >
            <div 
        ...
        <div className="flex flex-col">
          ...
          >
            <div className="flex items-center gap-3 mb-4">
              <span className="px-3 py-1 rounded-full bg-amber-500/10 text-amber-500 border border-amber-500/20 text-xs font-bold tracking-wider uppercase">
                {asset.assetType}
              </span>
            ...
            <div className="p-6 rounded-2xl bg-white/5 border border-white/10 mb-8 flex flex-col sm:flex-row justify-between items-center sm:items-start gap-4">
              <div>
              ...
              <Link 
                to={`/checkout?assetId=${asset.id}`} 
                className="w-full sm:w-auto px-8 py-4 bg-amber-500 hover:bg-amber-400 text-black font-bold rounded-xl transition-all shadow-[0_0_20px_rgba(245,158,11,0.3)] hover:shadow-[0_0_30px_rgba(245,158,11,0.5)] text-center whitespace-nowrap"
              >
                Acquire via Crypto
              </Link>
            </div>
            ...
            <div className="mb-8">
              ...
              <p className="text-slate-400 leading-relaxed">
                {asset.description || "No description available for this luxury asset. Contact our concierge for detailed specifications and provenance reports."}
              </p>
            </div>
              ...
              {asset.assetType === 'Aircraft' && (
                <>
                  <div className="p-4 rounded-xl bg-[#0A0A0E] border border-white/5 flex flex-col gap-2">
                    ...
                    <span className="text-sm font-semibold text-white">{asset.range}</span>
                  </div>
                  <div className="p-4 rounded-xl bg-[#0A0A0E] border border-white/5 flex flex-col gap-2">
                    <Users className="w-5 h-5 text-slate-500" />
                    <span className="text-xs text-slate-500 uppercase tracking-wider">Passengers</span>
                    <span className="text-sm font-semibold text-white">{asset.passengers}</span>
                  </div>
                  <div className="p-4 rounded-xl bg-[#0A0A0E] border border-white/5 flex flex-col gap-2">
                    <Fuel className="w-5 h-5 text-slate-500" />
                    <span className="text-xs text-slate-500 uppercase tracking-wider">Speed</span>
                    <span className="text-sm font-semibold text-white">{asset.speed}</span>
                  </div>
                </>
              )}
              ...
              {asset.assetType === 'Vehicles' && (
                <>
                  <div className="p-4 rounded-xl bg-[#0A0A0E] border border-white/5 flex flex-col gap-2">
                    ...
                    <span className="text-xs text-slate-500 uppercase tracking-wider">0-60 mph</span>
                    <span className="text-sm font-semibold text-white">{asset.zeroToSixty}</span>
                  </div>
                  <div className="p-4 rounded-xl bg-[#0A0A0E] border border-white/5 flex flex-col gap-2">
                    <Gauge className="w-5 h-5 text-slate-500" />
                    <span className="text-xs text-slate-500 uppercase tracking-wider">Top Speed</span>
                    <span className="text-sm font-semibold text-white">{asset.topSpeed}</span>
                  </div>
                  <div className="p-4 rounded-xl bg-[#0A0A0E] border border-white/5 flex flex-col gap-2">
                    <Fuel className="w-5 h-5 text-slate-500" />
                    <span className="text-xs text-slate-500 uppercase tracking-wider">Horsepower</span>
                    <span className="text-sm font-semibold text-white">{asset.horsepower} hp</span>
                  </div>
                </>
              )}
            </div>
